describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    let textProperty = await button.getProperty('innerText');
    let textValue = await textProperty['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let allItems = await page.$$('product-item');
    for(let i = 1; i < allItems.length; i++){
      let sRoot = await allItems[i].getProperty('shadowRoot');
      let button = await sRoot.$('button');
      await button.click();
    }
    let countProp = await page.$('#cart-count');
    let textProp = await countProp.getProperty('innerText');
    let count = await textProp['_remoteObject'].value;
